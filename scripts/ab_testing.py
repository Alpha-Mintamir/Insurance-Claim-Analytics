...
def prepare_group_dataa(data, group_column, gender_column='Gender', group_a_label='Female', group_b_label='Male'):
    # Group by gender and select the risk column (group_column)
    grouped_data = data.groupby(gender_column)[group_column]
    
    # Get data for females and males
    group_A = grouped_data.get_group(group_a_label)
    group_B = grouped_data.get_group(group_b_label)
